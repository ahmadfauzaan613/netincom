...
export default function ServicesGrid() {
  ...
  const services = [
    { title: 'ANNUAL REPORT & SUSTAINABILITY REPORT', image: '/images/Links/mockup_ar_pnre_isi.webp' },
    { title: 'COMPANY PROFILE', image: '/images/Links/po mockup_1.webp' },
    { title: 'MANUAL GCG', image: '/images/Links/gcg doss mockup.webp' },
    { title: 'RENCANA JANGKA PANJANG PERUSAHAAN (RJPP)', image: '/images/Links/RJPP.webp' },
    { title: 'OTHER BOOKLET & MAGAZINE', image: '/images/Links/PO mockup.webp' },
    { title: 'CALENDAR DESIGN AND PRODUCTION', image: '/images/Links/desk-calendar-with-texts-english.webp' },
    { title: 'PHOTO AND VIDEO', image: '/images/Links/arrangement-professional-photographer-equipment.webp' },
  ]
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-6 gap-6 items-start my-10">
        {services.map((service, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: i * 0.1 }} viewport={{ once: true }} className="relative w-full h-[25vh] rounded-lg overflow-hidden group shadow-md">
            <Image src={service.image} alt={service.title} fill loading="lazy" className="object-cover transition-transform duration-500 scale-125 group-hover:scale-150 " />
