  ...
  return (
    <div className={`grid  grid-cols-1 md:grid-cols-2 gap-12 my-32 items-center px-6 md:px-16`}>
      <motion.div initial={{ opacity: 0, x: -40 }} {...(path === '/' ? { whileInView: { opacity: 1, x: 0 }, viewport: { once: true } } : { animate: { opacity: 1, x: 0 } })} transition={{ duration: 0.8, ease: 'easeOut' }} className={`${path === '/' ? 'w-full md:w-10/12' : 'w-full md:w-11/12'}`}>
        <h1 className="text-4xl titleCustom md:text-5xl font-bold  leading-tight text-[#14462C]">ABOUT US</h1>
        <p className="text-base bodyCustom md:text-lg leading-relaxed pt-5 text-gray-700">
      ...
      <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="relative w-full h-[30vh] md:h-[40vh] rounded-2xl overflow-hidden shadow-xl">
